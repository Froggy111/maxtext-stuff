...
from maxtext.MaxText import inference_utils
...
class MaxEngine(engine_api.Engine):
  ...
  def prefill(
    ...
    with self._mesh, nn_partitioning.axis_rules(self.config.logical_axis_rules):
      flat_logits, new_vars = self.model.apply(
        params,
        jnp.expand_dims(padded_tokens, 0),
        jnp.expand_dims(token_positions, 0),
        decoder_segment_ids = jnp.expand_dims(attention_mask, 0),
        enable_dropout=False,
        model_mode=common_types.MODEL_MODE_PREFILL,
  ...
  @functools.partial(jax.jit, static_argnums=(0, 3, 4, 5, 6), donate_argnums=(2,))
  def generate(
      self,
      params: Params,
      decode_state: DecodeState,
      sampling_strategy: Literal["greedy", "weighted", "nucleus", "topk"] = "greedy",
      topk: int = 50,
      nucleus_topp = 0,
      temperature = 1.0
    ...
    new_token = inference_utils.sampling(previous_logits, self.rng, sampling_strategy,
                                        topk=topk,
