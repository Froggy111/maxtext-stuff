...
class Embed(nn.Module):
  ...
  def setup(self):
    ...
    # self.embedding.block_until_ready()
    # print(self.embedding)
    # jax.debug.print("full embedding matrix: {x}", x=self.embedding)
    # jax.debug.print("first embedding vector: {x}", x=self.embedding[0])
    # jax.debug.print("32000th embedding vector: {x}", x=self.embedding[31999])
    # jax.debug.print("16000th embedding vector: {x}", x=self.embedding[15999])
    # jax.debug.print("last embedding vector: {x}", x=self.embedding[-1])
  ...
  def __call__(self, inputs: Array) -> Array:
    ...
    """
    # jax.debug.print("embedding input positions: {x}", x=inputs)
    cfg = self.config
    ...
    # jax.debug.print("embedded matrix: {x}", x=output)
    return output
