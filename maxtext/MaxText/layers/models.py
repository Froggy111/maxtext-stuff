...
class Decoder(nn.Module):
  ...
  def __call__(self,
    ...
    # jax.debug.print("y before norm: {y}", y=y)
    ...
    # jax.debug.print("y after norm: {y}", y=y)
    y = nn.Dropout(rate=cfg.dropout_rate, broadcast_dims=(-2,))(
        y, deterministic=deterministic
    )
    # jax.debug.print("y after dropout: {y}", y=y)
    ...
    if cfg.logits_via_embedding:
      ...
      logits = linears.DenseGeneral(
          ...
          name='logits_dense',
          for_logits=True,
          broadcasted_size=cfg.vocab_size)(y) # We do not quantize the logits matmul.
    # jax.debug.print("logits before constraint: {x}", x=logits)
    logits = nn.with_logical_constraint(
        logits, ('activation_batch', 'activation_length', 'activation_vocab'))
    logits = logits.astype(jnp.float32)
    # jax.debug.print("output logits: {x}", x=logits)
    return logits
