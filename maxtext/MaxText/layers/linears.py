...
class DenseGeneral(nn.Module):
  ...
  def __call__(self, inputs: Array) -> Array:
    ...
    if self.for_logits:
      inputs = inputs[..., :features[0]]
    output = compute_dot_general(inputs, kernel, axis, contract_ind)
    if self.for_logits:
      print(f"FOR LOGITS, ORIGINAL OUTPUT SHAPE {output.shape}")
      to_concat = jnp.zeros((output.shape[0], output.shape[1], self.broadcasted_size - output.shape[2]))
      output = jnp.concatenate((output, to_concat), axis = 2)
      print(f"FOR LOGITS, RESHAPED OUTPUT TO {output.shape}")
