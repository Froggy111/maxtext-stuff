...
def _load_sentencepiece_tokenizer(tokenizer_path: str,
                                  add_bos: bool = False,
                                  add_eos: bool = True,
                                  reverse: bool = False):
...
def load_tokenizer(tokenizer_path: str, add_bos=False, add_eos=True):
  ...
  try:
    ...
    sp_tokenizer = AutoTokenizer.from_pretrained(tokenizer_path, add_special_tokens=True)
    return sp_tokenizer
...
class TokenizeOp:
  ...
  def __call__(self, features: Features) -> Features:
    for k in self.data_keys:
      # features[k] = tokenize(features[k])
      # print(features[k])
      # print(type(features[k]))
      # print(str(features[k]))
      # print(features[k])
      # print(features[k].decode("utf-8"))
      features[k] = self.sp_tokenizer(features[k], return_tensors='np')
    return features
